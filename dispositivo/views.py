...
from .forms import ComputadorForm, RoteadorForm, MemoriaRamForm
        ...
        if form.is_valid():
            ...
            try:
                ...
                try:
                    ...
                    if ip_data is not None:
                        EnderecoIp(ip_address=ip_data,
                        content_type_id=roteador_tipo.id, parent_object_id=pc_novo.id).save()
                    if mac_data is not None:
                        EnderecoMac(mac_address=mac_data,
                        content_type_id=roteador_tipo.id, parent_object_id=pc_novo.id).save()
...
def impressora_view(request, pagina):
    ...
    if pesquisa != '' and pesquisa is not None:
        impressoras = impressoras.filter(
            Q(patrimonio__icontains=pesquisa) | Q(ip_impressora__ip_address__icontains=pesquisa) | Q(departamento__departamento__icontains=pesquisa) | Q(local__icontains=pesquisa) | Q(id__iexact=pesquisa)
        )
